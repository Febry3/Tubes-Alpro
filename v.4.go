		...
		if ans == 1 {
			...
			if idx == -1 {
				...
				if A.Pabrik[idx].arrMobil.n < NMAX {
					...
					if isSame(*A, tMob.namaMobil, tMob.tahunKeluar) {
						fmt.Println("Data sudah ada")
					} else {
						pilihan()
						fmt.Scan(&ans1)
...
func isSame(A arrPabrikan, nama string, tahun int) bool {
	for i := 0; i < A.n; i++ {
		for j := 0; j < A.Pabrik[i].arrMobil.n; j++ {
			if A.Pabrik[i].arrMobil.M[j].namaMobil == nama && A.Pabrik[i].arrMobil.M[j].tahunKeluar == tahun {
				return true
			}
		}
	}
