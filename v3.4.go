		...
		if ans == 1 {
			...
			if idx == -1 {
				...
				if A.Pabrik[idx].arrMobil.n < NMAX {
					...
					fmt.Scan(&tMob.M[tMob.n].namaMobil, &tMob.M[tMob.n].jumlahUnit, &tMob.M[tMob.n].jumlahPenjualan, &tMob.M[tMob.n].tahunKeluar)
					tMob.M[tMob.n].namaPabrikan = A.Pabrik[idx].nama
					tMob.n++
				} else {
					fmt.Println("Data sudah penuh")
				}
