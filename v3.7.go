	...
	for i := 0; i < A.n; i++ {
		for j := 0; j < A.Pabrik[i].arrMobil.n; j++ {
			if A.Pabrik[i].arrMobil.M[j].namaMobil == target {
				temp = "A.Pabrik[i].arrMobil.M[j].namaMobil" + "A.Pabrik[i].arrMobil.M[j].namaPabrikan" + "A.Pabrik[i].arrMobil.M[j].jumlahUnit" + "A.Pabrik[i].arrMobil.M[j].jumlahPenjualan" + "A.Pabrik[i].arrMobil.M[j].tahunKeluar"
			}
		}
	}
	if temp == "" {
	...
	for ans != 3 {
		...
		if ans == 1 {
			...
			if A.n == 0 {
				...
				temp1 = cariMobilSemuaPabrik(A, temp)
...
func deleteData(A *arrPabrikan) {
	var ans, idx, ans1, idxM, ans2 int
	var t, tMob string
	var tPab pabrikan
		...
		if ans == 1 {
			...
			if idx == -1 {
				...
				if idxM == -1 {
					...
					fmt.Scan(&ans2)
					if ans2 == 1 {
						for i := idxM; i <= A.Pabrik[idx].arrMobil.n; i++ {
							A.Pabrik[idx].arrMobil.M[i] = A.Pabrik[idx].arrMobil.M[i+1]
						}
						A.Pabrik[idx].arrMobil.n--
						fmt.Println("Data berhasil dihapus")
					}
...
//bagian edit data pabrikan tidak terupdate
