		...
		if ans == 1 {
			...
			if idx == -1 {
				...
				if A.Pabrik[idx].arrMobil.n < NMAX {
					...
					if isSame(*A, tMob.namaMobil, tMob.tahunKeluar, tMob.namaPabrikan) {
						fmt.Println("Data sudah ada")
					} else {
						pilihan()
						fmt.Scan(&ans1)
...
func editData(A *arrPabrikan) {
	...
	for ans != 3 {
		...
		if ans == 1 {
			...
			fmt.Print("Masukkan nama mobil yang ingin diedit : ")
			fmt.Scan(&t)

			idx, idxM = cariIDXMobilSemuaPabrik(*A, t)
...
func uiCari() {
	...
	fmt.Println("3. Cari Jumlah Unit Mobil")
...
func top3(A arrPabrikan) {
	...
	if NMAX >= 3 {
		...
		fmt.Println("Size array kurang cukup untuk menampilkan top 3")
	}
}

func cariData(A arrPabrikan) {
	var ans, idx, i1, i2, tempInt int
	var temp string
	var tempP pabrikan
	...
	for ans != 4 {
		...
		if ans == 1 {
			...
			if A.n == 0 {
				...
				fmt.Print("Masukkan jumlah unit mobil yang ingin dicari datanya: ")
				fmt.Scan(&tempInt)

				cariUnitMobil(A, tempInt)
			}
		}
	...
	for ans != 3 {
		...
		if ans == 1 {
			...
			if i1 != -1 && i2 != -1 {
				...
				fmt.Scan(&ans1)
				if ans1 == 1 {
					for i := i2; i < A.Pabrik[i1].arrMobil.n; i++ {
						if i == A.Pabrik[i1].arrMobil.n-1 {
							A.Pabrik[i1].arrMobil.M[i] = dumMobil
						} else {
							A.Pabrik[i1].arrMobil.M[i] = A.Pabrik[i1].arrMobil.M[i+1]
						}
...
func isSame(A arrPabrikan, nama string, tahun int, pabrik string) bool {
	for i := 0; i < A.n; i++ {
		for j := 0; j < A.Pabrik[i].arrMobil.n; j++ {
			if A.Pabrik[i].arrMobil.M[j].namaMobil == nama && A.Pabrik[i].arrMobil.M[j].tahunKeluar == tahun && A.Pabrik[i].arrMobil.M[j].namaPabrikan == pabrik {
				return true
			}
		}
	}
...
func isSamePabrik(A arrPabrikan, pabrik string) bool {
	for i := 0; i < A.n; i++ {
		if pabrik == A.Pabrik[i].nama {
			return true
		}
	}
	return false
...
//binary search
func cariUnitMobil(A arrPabrikan, target int) {
	var B arrMobil
	var iMax int
	var temp mobil
	gabungSemuaMobil(A, &B)
	...
	for left <= right && !found {
		mid = (left + right) / 2
		if target == B.M[mid].jumlahUnit {
			found = true
			fmt.Println(B.M[mid].namaMobil, B.M[mid].namaPabrikan, B.M[mid].jumlahUnit, B.M[mid].jumlahPenjualan, B.M[mid].tahunKeluar)
		} else if target < B.M[mid].jumlahUnit {
			right = mid - 1
		} else {
			left = mid + 1
		}
